"""Verification experiment configuration loader."""
...
def load_verification_config() -> dict[str, Any]:
    """Load the full verification YAML configuration."""
    return load_yaml(_CONFIG_PATH)


def get_experiment_config(experiment_id: str) -> dict[str, Any]:
    """Return the config block for a single experiment (e.g. 'VZ3-01').

    The returned dict includes an ``experiment_id`` key so that scripts
    never need to hardcode their own ID.
    """
    full = load_verification_config()
    if experiment_id not in full:
        raise KeyError(f"Experiment '{experiment_id}' not found in verification.yaml")
    cfg = dict(full[experiment_id])
    cfg["experiment_id"] = experiment_id
    return cfg


__all__ = ["get_experiment_config", "load_verification_config"]
