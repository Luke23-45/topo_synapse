"""Deployment test CLI script for Z3 SYNAPSE models.

Uses Hydra + OmegaConf for production-grade experiment management.

Usage:
    python -m synapse.synapse.scripts.deploy
    python -m synapse.synapse.scripts.deploy checkpoint.path=path/to/best.pt
"""
...
@hydra.main(
    config_path="../../../config",
    config_name="deploy/default",
    version_base="1.3",
)
def main(cfg: DictConfig) -> int:
    """Run exact-audit deployment test using Hydra configuration."""
    ...
    sequences, _, _ = generate_topology_dataset(
        cfg.data.num_samples,
        length=model_cfg.max_history_tokens,
        noise_std=cfg.data.noise_std,
        seed=cfg.execution.seed + 99,
    )
    seq = torch.from_numpy(sequences).float()

    # Run exact audit
    audit = model.exact_audit(seq)[0]

    # Output path
    output_path = Path(cfg.output.path)
    if "hydra" in cfg:
        output_path = Path(cfg.hydra.run.dir) / "deploy.json"

    # Save results
    if cfg.output.save_audit:
