from __future__ import annotations

from time import perf_counter
from typing import Any

import numpy as np

from synapse.empirical.common.baselines import proxy_topology_features
from synapse.empirical.common.math_utils import ridge_probe_accuracy
from synapse.empirical.common.shared import ArtifactWriter, build_empirical_model_from_config


def run_experiment(
    config: dict[str, Any],
    writer: ArtifactWriter,
    verbose: bool = False,
) -> dict[str, Any]:
    p = config["params"]
    base_seed = p["base_seed"]
    grid = p["grid"]
    accuracy_threshold = p["accuracy_threshold"]

    t0 = perf_counter()
    rows: list[dict[str, Any]] = []

    for entry in grid:
        K_val = entry["K"]
        r_val = entry["r"]
        seed = base_seed + K_val + r_val

        cfg_copy = dict(config)
        cfg_copy["params"] = dict(p)
        cfg_copy["params"]["seed"] = seed
        cfg_copy["model"] = dict(config.get("model", {}))
        cfg_copy["model"]["K"] = K_val
        cfg_copy["model"]["r"] = r_val

        model, bundle = build_empirical_model_from_config(cfg_copy)
        train_x = proxy_topology_features(model, bundle.train_sequences)
        test_x = proxy_topology_features(model, bundle.test_sequences)
        acc = ridge_probe_accuracy(train_x, bundle.train_labels, test_x, bundle.test_labels)

        row = {
            "K": K_val,
            "r": r_val,
            "seed": seed,
            "accuracy": acc,
            "above_threshold": acc >= accuracy_threshold,
        }
        rows.append(row)
        writer.log_row("results.jsonl", **row)

    writer.write_csv("results.csv", rows)

    elapsed = perf_counter() - t0
    summary = {
        "experiment_id": config["experiment_id"],
        "experiment_name": config["experiment_name"],
        "formal_reference": config["formal_reference"],
        "claim": config["claim"],
        "duration_seconds": elapsed,
        "total_cases": len(rows),
        "all_above_threshold": all(r_["above_threshold"] for r_ in rows),
    }
    return summary


if __name__ == "__main__":
    raise SystemExit(
        __import__("synapse.empirical.common.shared", fromlist=["run_standalone"]).run_standalone(
            experiment_id="EZ3-09",
            run_experiment_fn=run_experiment,
        )
    )
